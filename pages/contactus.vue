<template>
  <div>
    <BreadCrumbs class="md:pt-0 pt-3" title="Contact Us" :paths="data.breadcrumbs" />
    <div class="text-theme-text-primary body-font">
      <div class="w-full flex md:flex-row flex-col md:space-y-0 space-y-4 md:px-12 px-2 md:py-10 py-2">
        <div
        ...
        <div class="md:w-1/2 w-full flex flex-col justify-center md:-mt-auto md:ml-auto md:px-10 px-2 md:py-5 py-2">
          <h2 class="text-theme-text-primary text-2xl mb-1 title-font font-bold">
            {{ data.content.heading }}
          </h2>
          <p class="leading-relaxed mb-5 text-theme-text-primary" v-html="data.content.subheading"></p>
          <form ref="form" @submit="submitdata">
            ...
            <div v-if="error" class="px-4 py-2 bg-red-500 text-white border border-red-700 rounded-md">
              {{ error }}
            </div>
            <div class="flex justify-center md:justify-start py-5 w-full">
              <component is="button" type="submit" variant="primary"
                class="px-6 bg-theme-primary-500 rounded-md py-2 text-white" :disabled="loading"
...
const validate = () => {
  const fields = [inputName, inputEmail, inputPhone, inputWebsiteUrl, inputMessage]
