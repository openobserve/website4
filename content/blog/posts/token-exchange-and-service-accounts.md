...
description: "We are thrilled to announce the launch of our latest feature, Service Accounts in OpenObserve! Ensuring enhanced security and programmatic access to OpenObserve APIs."
...
## Making APIs accessible 
 OpenObserve introduces **service accounts** as a secure way to invoke APIs. OpenObserve supports two types of service accounts:
- **Native Service accounts**
- **SSO service accounts**

Service accounts are designed exclusively for **programmatic access to OpenObserve APIs** and come with the following features:
  ...
  - Service accounts are bound by a per-user search quota and also consumed from the   overall cluster search quota.
...
In Local Auth Mode, you can create **Native Service Accounts** by following the steps below:

1. Navigate to **IAM**
2. Select **Service Accounts**.
3. **Add a Service Account**: Click on the `Add Service Account` button.
4. **Fill in the Details**: Enter the required information, Email, First Name and Last Name. 

- ![add service account](/img/blog/token-exchange/add-service-account.png)

5. **Retrieve Your Token**: Once the service account is created, you will receive a unique token. You can then use the generated service account token to securely invoke OpenObserve APIs.
6. Implement **ReBAC Controls**: Set up Relationship-Based Access Control to define what actions the service account can perform within OpenObserve.
   - Navigate to IAM -> Roles -> Add Role.
   - Select Appropriate Permissions.
   - Add `Service Accounts` to the roles.
   - Click Save.

- ![permissions](/img/blog/token-exchange/permissions.png)
