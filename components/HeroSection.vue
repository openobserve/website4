<template>
  <div :style="{ backgroundImage: `url(${data.backgroundImage})` }" class="bg-no-repeat  bg-center py-10"
    :class="bgClasses[backgroundBgType]">
    <div class="flex flex-col px-4 md:flex-row w-full items-stretch container mx-auto" :class="directions[direction]">
      <div class="flex flex-col justify-center w-full md:w-1/2 py-4 md:py-12 lg:py-12 md:px-6 lg:px-10">
        <div v-if="data.tagline" class="text-center md:text-left text-sm mb-6">
          <div class="bg-theme-primary-100 rounded-full text-black px-4 py-2 inline-block" v-html="data.tagline"></div>
        </div>
        ...
        <div
          ...
          <Button v-if="data.btnText2" variant="requestdemo" :to="data.btnTo2">
            {{ data.btnText2 }}
          </Button>
        </div>
      ...
      <div class="w-full md:w-1/2 -mt-10 md:mt-0">
        <img :src="data.image" :alt="data.title" class="object-center object-contain" />
      </div>
    </div>
  </div>
</template>
